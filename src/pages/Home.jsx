...
function Home() {
  ...
  const moviesNewsEndpoint = getNewsCategoriesEndpoint("movies", 1, 6);
  ...
  let moviesData = useFetch(moviesNewsEndpoint);
  ...
  const adaptedMoviesData = getNewsList(moviesData);
    ...
    <Layout>
      ...
      <section className="movies my-5">
        <Container>
          ...
          <NewsCardList newsList={adaptedMoviesData} />
          <p>
            <Link to="/category/movies" className="text-secondary">
              Movies
            </Link>
            .
          </p>
